...
<body class="bg-gradient-to-br from-gray-900 via-gray-800 to-black text-gray-100 min-h-screen flex flex-col">
  ...
  <main class="flex-1 max-w-4xl mx-auto px-6 py-10">
    <h2 class="text-xl font-bold mb-4 text-green-400 text-center">Global Leaderboards</h2>
    ...
    <table class="min-w-full table-auto border-collapse">
      ...
      <tbody class="divide-y divide-gray-700 text-center">
        ...
        <tr>
          <td class="p-2"><?= ($start_rank ?? 0) + $i + 1 ?></td>
          <td class="p-2"><?= html_escape($row['username'] ?? 'Unknown') ?></td>
          <td class="p-2"><?= $row['score'] ?? 0 ?></td>
          <td class="p-2"><?= html_escape($row['board_size'] ?? 'N/A') ?></td>
        </tr>
        <?php endforeach; ?>

        <?php if (empty($scores)): ?>
        <tr>
          <td colspan="4" class="text-center p-4 text-gray-400">No scores found.</td>
        </tr>
        <?php endif; ?>
      </tbody>
    </table>
